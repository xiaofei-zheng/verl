#!/bin/bash
# Qwen3-235B-A22B GRPO RL — 4-node Megatron + sglang (训练专用，不存 checkpoint)
# 可在初始环境 rayjob 上直接提交，自带源码补丁
set -xeuo pipefail

cd /shared_nfs/xiaofei/verl

# ============================================================
# 源码补丁: enable_memory_saver 从 hardcoded False → 读配置
# 这是训练必须的，否则 sglang 不释放显存会 OOM
# ============================================================
python3 - <<'PATCH_SGLANG'
import re, pathlib
...
if old in src:
    ...
    print("[patch] async_sglang_server.py: already patched, skipping")
else:
...
# ---- Official 4-node (32 GPU) parallelism config ----
NNODES=4
GPUS_PER_NODE=8
...
exp_name="grpo_sglang_4node_pp${train_pp}_tp${train_tp}_ep${EP}_train_only"
...
dp = world_size // (tp * pp)
print(f"[preflight] nnodes=$NNODES, world={world_size}, tp={tp}, pp={pp}, ep={ep}, dp={dp}")
...
python3 -m verl.trainer.main_ppo \
    ...
    2>&1 | tee /shared_nfs/xiaofei/verl/megatron_235b_4node_train_only.log
