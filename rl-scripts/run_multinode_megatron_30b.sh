#!/bin/bash
# Qwen3-30B-A3B GRPO RL — multi-node Megatron + sglang
...
MODEL_PATH=${MODEL_PATH:-/shared_nfs/xiaofei/models/Qwen3-30B-A3B}
SAVE_DIR=${SAVE_DIR:-/shared_nfs/xiaofei/verl/checkpoints/qwen3_30b_megatron/grpo_sglang}
...
# DP = total_gpus / (TP * EP * PP)
DP=$(( NNODES * GPUS_PER_NODE / (TRAIN_TP * TRAIN_EP * TRAIN_PP) ))

# ── Batch config (scale with DP) ──
# base: 64 prompts per DP rank → batch = 64 * DP
TRAIN_BATCH_SIZE=$(( 64 * DP ))
PPO_MINI_BATCH_SIZE=$(( 64 * DP ))
ROLLOUT_N=8
MICRO_BATCH=4
...
print(f"[preflight] OK: nnodes=$NNODES, world={world_size}, tp={tp}, ep={ep}, "
      f"dp={dp}, n={n}, batch={train_batch_size}, mini_batch={ppo_mini_batch_size}, "
      f"micro_batch=$MICRO_BATCH, log_prob_micro=$LOG_PROB_MICRO_BATCH, lr=$LR")
PY
...
python3 -m verl.trainer.main_ppo --config-path=config \
    ...
    trainer.experiment_name=grpo_sglang_${NNODES}node \
    trainer.n_gpus_per_node=$GPUS_PER_NODE \
    trainer.nnodes=$NNODES \
    trainer.save_freq=50 \
    trainer.test_freq=-1 \
    trainer.total_epochs=1 \
    2>&1 | tee /shared_nfs/xiaofei/verl/megatron_30b_${NNODES}node.log
